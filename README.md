...
- **A `googtag` with an `AW-` `tagId` does NOT send GA4 page views, even with `measurement_id` in its `configSettingsTable`** — that param is not a destination; GA4 gets nothing. `gaawe` event tags with `measurementIdOverride` still work, which is what makes it hide: the property shows `session_start`, `first_visit`, and click/lead events but **zero `page_view`**, so landing pages read `(not set)`, page paths are empty, and sessions collapse to only visitors who clicked something. Every GA4 destination needs its own `googtag` with a `G-` `tagId` on All Pages. Found on Dan Wallen's unified `GTM-MM8PL8KW` 2026-09-09 — silent since 2025-11-22 across six sites.
